...
ReverbFlangeWidget::ReverbFlangeWidget(bool isParallel,bool isSymphonic, QWidget *parent) :
    EffectEditBaseWidget(parent)
{
    ...
    {
        if(isSymphonic)
            mainlyt->addWidget(new QLabel(tr("Reverb/Symphonic")), 0, 1);
        else
            mainlyt->addWidget(new QLabel(tr("Reverb/Flange")), 0, 1);
    }
    else
    ...
    QGroupBox *chorusGroupBox = new QGroupBox(isSymphonic?"Symphonic":"Flange");
    ...
    if( ! isSymphonic)
    {
        chorusLayout->addWidget(new QLabel(tr("FB Gain")), 0, 3);
        spinBox = new QSpinBox();
        ...
        chorusLayout->addWidget(spinBox, 1, 3);
    }

    chorusLayout->addWidget(new QLabel(tr("Depth")), 2, 1);
    spinBox = new QSpinBox();
    spinBox->setMinimum(0);
    spinBox->setMaximum(100);
    spinBox->setSuffix(" %");
